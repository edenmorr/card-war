...
using namespace std;
   ...
   TEST_CASE("Test number 3: ") { //at the begining of the game both have the same amount of cards
    ...
    Player p2("Bob");
    Game game(p1, p2);
    CHECK(p1.cardesTaken() >p2.stacksize());
    CHECK(p2.stacksize() == 0);
}
...
TEST_CASE("Test number 5: ") {//"cheack players' cards after 10 turns"
    ...
    int s=p1.stacksize()+p2.stacksize();
    CHECK(( s == 52)); 
    CHECK_EQ(p1.stacksize(), p2.stacksize());
}
   TEST_CASE("Test number 6: ") {//"cheack stacksize after 2 turns 
    ...
    Player p2("Bob");
    Game game(p1, p2);
    int stacksize1 = game.p1.stacksize();
    int stacksize2 = game.p2.stacksize();
        game.playTurn();
    CHECK(game.p1.stacksize() == (stacksize1 - 1));
    CHECK(game.p2.stacksize() == (stacksize2 - 1) );
        game.playTurn();
    CHECK(game.p1.stacksize() == (stacksize1 - 1));
    CHECK(game.p2.stacksize() == (stacksize2 - 1) );
   }
   ...
   TEST_CASE("Test number 6: ") {//"cheack stacksize after 5 turns (what that raight in the demo)
    ...
    CHECK(game.p1.stacksize() == 21) ;
    CHECK(game.p2.stacksize() == 21 );
    CHECK(game.p1.stacksize() < 21); 
    CHECK(game.p2.stacksize() < 21);  
   }
